...
Additional documentation for various zxweather components is available in the following manuals:
...
Because of this architecture setting up the web interface on a remote system on another network (such as a VPS) can be difficult. If using a VPN or an SSH tunnel is impractical then the web interface does provide a very basic database replication facility which is described in chapter \ref{cha_db_replication}.
