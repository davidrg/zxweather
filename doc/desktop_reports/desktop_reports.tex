...
\newcommand{\doccopyright}{\textcircled{c} Copyright David Goodwin, 2018.}
...
\hline
\verb|format| & The output format. Either table, html or text.\\
...
Plain text outputs can generate and display plain text documents (figure \ref{img_text_output}) which are shown to the user with a fixed with font. If the output type is \verb|save| rather than \verb|show| this output format could be used to generate other document formats.
...
Where the \verb|format| parameter is either \verb|html| for html output or \verb|text| for plain text output. The \verb|template| parameter specifies the mustache template to produce output with. 
