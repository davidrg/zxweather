...
def parse_sample(sample):
    ...
    s = Sample(station_code=parts[0],
               ...
               pressure=noneable_float(parts[7]),
               wind_speed=noneable_float(parts[8]),
               gust=noneable_float(parts[9]),
               wind_direction=noneable_float(parts[10]),
               rainfall=noneable_float(parts[11]),

               # Davis
               record_time=noneable_int(parts[12]),
               record_date=noneable_int(parts[13]),
               high_temperature=noneable_float(parts[14]),
               low_temperature=noneable_float(parts[15]),
               high_rain_rate=noneable_float(parts[16]),
               solar_radiation=noneable_float(parts[17]),
               wind_sample_count=noneable_int(parts[18]),
               gust_wind_direction=noneable_float(parts[19]),
               average_uv_index=noneable_float(parts[20]),
               evapotranspiration=noneable_float(parts[21]),
               high_solar_radiation=noneable_float(parts[22]),
               high_uv_index=noneable_float(parts[23]),
               forecast_rule_id=noneable_int(parts[24]),

               # WH1080
               sample_interval=noneable_int(parts[25]),
               record_number=noneable_int(parts[26]),
               last_in_batch=noneable_bool(parts[27]),
               invalid_data=noneable_bool(parts[28]),
               wh_wind_direction=noneable_str(parts[29]),
               total_rain=noneable_float(parts[30]),
               rain_overflow=noneable_bool(parts[31]))
    return s
...
def sample_to_dict(sample, station_id):
    return {
        ...
        "absolute_pressure": sample.pressure,
