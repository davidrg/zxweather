  ...
  select
    ...
    from (
      select   -- Various aggregates per day
        ...
        sum(rainfall)             AS tot_rain,
        avg(average_wind_speed)   AS avg_wind,
        max(gust_wind_speed)      AS max_wind,
...
left outer join (  -- This is unit-converted report data, d:
  select
    ...
    -- Wind table
    case when p.kmh then data.avg_wind * 3.6
         when p.mph then data.avg_wind * 2.23694
         else            data.avg_wind                                                end as avg_wind,
    case when p.kmh then data.max_wind * 3.6
         when p.mph then data.max_wind * 2.23694
