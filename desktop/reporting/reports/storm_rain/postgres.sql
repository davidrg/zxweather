...
--drop index idx_rain_time_station;
...
with marked_storm_rows as (
  select
    ...
    when nc.next - s.time_stamp > '24 hours':: interval
    ...
    when s.time_stamp - cp.prev > '24 hours'::interval
...
inner join sample s on s.time_stamp between storm.storm_start_time and storm.storm_end_time
  ...
  where storm.storm_start_time between :start and :end
group by storm.storm_start_time, storm.storm_end_time, storm.station_id
having sum(s.rainfall) >= 0.4 -- Filter out storms smaller than 0.4mm - isolated rain clicks don't count
