WITH parameters AS (
   SELECT
     ...
     date_trunc('month', :atDate::date)  AS date,
...
SELECT
  ...
  lpad(to_char(records.high_temp_time, 'fmDD'), 4, ' ')			AS high_temp_time,
  lpad(round((
	case when p.fahrenheit then records.low_temp * 1.8 + 32
		 else records.low_temp
	end)::numeric, 1)::varchar, 4, ' ')									AS low_temp,
  lpad(to_char(records.low_temp_time, 'fmDD'), 4, ' ')				AS low_temp_time,
  lpad(round((
  ...
  lpad(to_char(records.high_wind_time, 'fmDD'), 4, ' ' )			AS high_wind_time,
  lpad(point.point::varchar, 3, ' ')									AS dom_wind_dir
from parameters p
  inner join station stn on lower(stn.code) = p.station_code
LEFT OUTER JOIN (
  SELECT
  ...
  FROM (SELECT
        ...
        FROM (sample s
          JOIN (SELECT
                  date_trunc('month', s.time_stamp):: DATE                 AS date_stamp,
                  s.station_id,
                ...
                FROM (
                  ...
                  where date_trunc('month', x.time_stamp) = p.date
                ) s, parameters p
                where date_trunc('month', s.time_stamp)::date = p.date::date
                GROUP BY date_trunc('month', s.time_stamp)::date, s.station_id
               ) dr ON (date_trunc('month', s.time_stamp)::date = dr.date_stamp))
...
) as records on records.date = p.date and records.station_id = stn.station_id
LEFT OUTER JOIN (
    WITH wind_directions AS (
        ...
        where date_trunc('month', s.time_stamp)::date = p.date and s.wind_direction is not null
    ...
    FROM wind_directions d
      INNER JOIN max_count mc ON mc.station_id = d.station_id AND mc.max_count = d.count
    GROUP BY d.station_id
    ) as dwd on dwd.station_id = records.station_id
