...
void WebDataSource::fetchSamples(SampleColumns columns,
    ...
    progressDialog->setLabelText("Loading system configuration");
    progressDialog->show();

    QNetworkRequest request(baseURL + "data/sysconfig.json");
...
//            failedDataSets.append(reply->request().url().toString());
    ...
    case DLS_INIT: {
        if (LoadStationConfigData(reply->readAll())) {
            ...
            emit sampleRetrievalError("Failed to load system configruation.");
            dlReset();
            return;
        }
        break;
...
data_file_t WebDataSource::loadDataFile(QString url, QStringList fileData,
    ...
    dataFile.hasSolarData = isSolarDataAvailable;
...
#else
            QString url = baseURL + QString::number(year) + "/" +
                    QString::number(month) + "/samples.txt";
#endif
